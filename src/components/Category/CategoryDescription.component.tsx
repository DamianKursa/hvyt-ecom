...
<p>Zobacz również gałki meblowe w naszej ofercie.</p>`,
  ...
  // Define type explicitly for categorySections
  const categorySections: Section[] = sections[category] || [
    { title: 'Brak opisu dla tej kategorii', content: '' },
  ];

  return (
    <section
      className={`bg-[#F5F5F5] py-[88px] ${fullWidth ? 'w-full' : 'container mx-auto max-w-[1440px]'}`}
      style={{ backgroundColor: 'var(--color-beige)' }}
    >
      <div className={`${fullWidth ? '' : 'container mx-auto'} px-10`}>
        <div className="flex flex-col md:flex-row items-center justify-center gap-8">
          {/* Text Column */}
          <div className="w-full md:w-1/2 text-left">
            {categorySections.map((section: Section, index: number) => (
              <ExpandableSection
                key={index}
                title={section.title}
